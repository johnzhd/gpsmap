...
global_db_control_collection = "control"

global_key_sign = "sign"
global_key_pwd = "pwd"
global_key_token = "token"
...
class opt():
    ...
    def check_token(self, token):
        try:
            db = self.connect.get_database(global_db_name)
            collection = db.get_collection(global_db_control_collection)
            now = datetime.datetime.now()
            ret = collection.find_one({global_key_token: token})
            if not ret or not ret['time']:
                return None
            record = string_to_time(ret['time'])
            delat = now - record
            
            if record > now or delat.total_seconds() > 60 * 15:
                collection.find_one_and_update({"_id": ret["_id"]}, {'$set': {'token': ""}})
                return None
            else:
                now = time_now()
                token = hashOne(now)
                collection.find_one_and_update({"_id": ret["_id"]}, {'$set': {'time': now, 'token': token}})
                return [token]
        except Exception as e:
            error_print(e)
        return None
    ...
    def login(self, user, pwd):
        try:
            ...
            collection = db.get_collection(global_db_control_collection)
            now = datetime.datetime.now()
            ret = collection.find_one({global_key_sign: user, global_key_pwd: pwd})
            if not ret:
                return None
            now = time_now()
            token = hashOne(now)
            print(">{0}-{1}  =>  {2}".format(user, pwd, token))
            collection.find_one_and_update({"_id": ret["_id"]}, {'$set': {'time': now, 'token': token}})
            return [token]
        except Exception as e:
            error_print(e)
        return None
    def add_control(self, user, pwd):
        try:
            db = self.connect.get_database(global_db_name)
            collection = db.get_collection(global_db_control_collection)
            ret = collection.insert_one({global_key_sign: user, global_key_pwd: pwd, 'token': "", 'time': ""})
            if ret and ret.inserted_id:
                return ["Add User {{{0}}} done".format(user)]
        except Exception as e:
            error_print(e)
        return None
...
def unique_Login(user, pwd, token):
    obj = get_unique_opt()
    if not obj:
        return None
    if user is None or len(user) == 0:
        if token is None or len(token) == 0:
            return None
        ret = obj.check_token(token)
    else:
        ret = obj.login(user, pwd)
    return ret

def unique_AddControl(user, pwd):
    obj = get_unique_opt()
    if len(user) > 0 and len(pwd) > 0:
        ret = obj.add_control(user, pwd)
        return ret
    return None

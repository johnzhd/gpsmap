...
def login(unuse):
    ...
    try:
        ...
        if len(password) > 0:
            password = hashOne(password)
        ret = backend.unique_Login(sign, password, None)
        if ret:
            data = {"success": 1, "data": ret}
            ret = json.dumps(data, indent=None)
            return ret
    except Exception as e:
        error_print(e)
...
def token(unuse):
    ...
    try:
        sign = args['sign']
        password = args['password']
        ret = backend.unique_Login(None, None, token)
        if ret:
            data = {"success": 1, "data": ret}
            ret = json.dumps(data, indent=None)
            return ret
...
def addControl(unuse):
    ...
    try:
        ...
        if len(password) > 0:
            password = hashOne(password)
        ret = backend.unique_AddControl(sign, password)
        if ret:
            data = {"success": 1, "data": ret}
            ret = json.dumps(data, indent=None)
            return ret
...
global_main_enter["login"] = login
global_main_enter["token"] = token

## need system identify
global_main_enter["becareful"] = becareful
global_main_enter["user"] = addControl
